...
`docs/design-v.1.3.md` 是在本版基础上，针对异构网络链路成本（跳数、带宽、确认开销）做的专项优化设计，不替代本版的总体架构边界定义。

评审建议：

1. 先读本版（v1.0/v1.1）确认职责边界与基础流程；
2. 再读 v1.3 关注“传输优化增量”；
3. 按下表逐项核对实现差异与迁移影响。

| 本文档章节 | v1.3 对应章节 | 差异说明 |
|---|---|---|
| `4.2 Sync 调用实现（阻塞 request/reply）` | `4. P1：减少逻辑跳数` | 从阻塞等待改为“异步请求 + 关联 ID 响应合并”，逻辑中转由 4 跳降为 2 跳 |
| `4.1 Topic 设计` / `4.2 Reply 语义` | `4.2 目标流程`、`5.3 Topic Alias` | 保留 topic 体系，但新增 `response/{request_id}` 思路与 MQTT 5.0 alias 优化 |
| `2.3 DD 责任边界`、`4.2 timeout 语义` | `4.3 接口改造建议`、`4.5 代价与兼容性` | `SendSync` 扩展为 `SendSyncAsync`，同步调用改为兼容包装层 |
| `6. 可靠性与性能职责矩阵` | `7. QoS 分层策略` | 由“统一 QoS”细化为关键流量 QoS1、非关键流量 QoS0 |
| `4.* envelope 与消息头约定` | `5.1 动态字段裁剪`、`5.2 全信封压缩` | 增加方向化序列化 profile 与压缩策略，降低每跳字节成本 |
| `5. 服务发现实现` | `6. P3 智能混合路由` | 服务发现模型扩展网络域信息，用于同域直连/跨域回退决策 |
| `9. 工程实现清单` | `8. 分阶段落地计划`、`9. 验收指标` | 由任务清单升级为阶段化实施 + SLO 验收口径 |
